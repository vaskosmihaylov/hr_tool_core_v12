...
class WorkerResource extends Resource
{
                ...
                Section::make('🏢 Месторабота')
                    ...
                    ->schema([
                        ...
                        Select::make('work_place_id')
                            ->label('Обект')
                            ->options(fn (Get $get): Collection => WorkPlace::query()
                                ->where('region_id', $get('region_id'))
                                ->where('status', WorkPlace::WORK_PLACE_ACTIVE)
                                ->pluck('name', 'id'))
                            ->required()
                            ->live()
                            ->afterStateUpdated(fn (Set $set) => $set('work_place_activity_id', null))
                            ->columnSpan(1),

                        Select::make('work_place_activity_id')
                            ->label('Дейност')
                            ->options(fn (Get $get): Collection => WorkPlaceActivity::query()
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ...
                Action::make('vacation')
                    ->label('Отпуска')
                    ->icon('heroicon-o-calendar')
                    ->color('info')
                    ->url(fn (Worker $record): string => route('service.worker.vacation', $record)),

                Action::make('bonus')
                    ->label('Бонус/Глоба')
                    ->icon('heroicon-o-currency-dollar')
                    ->color('warning')
                    ->url(fn (Worker $record): string => route('service.worker.bonus', $record)),

                Action::make('view_region')
                    ->label('Регион')
                    ->icon('heroicon-o-map')
                    ->color('info')
                    ->url(fn (Worker $record): string => "/service/regions/{$record->region_id}")
                    ->visible(fn (Worker $record): bool => $record->region_id !== null),
            ])
            ->bulkActions([
    ...
    {
        return static::getModel()::where('status', Worker::WORKER_ACTIVE)->count();
    }
