...
class ConfigureMonthlyHours extends Page implements HasForms
{
    ...
    public function mount(int $workplace, string $date): void
    {
        ...
        $this->isLocked = PresenceConfigurationService::isMonthLocked(
            $this->workplace,
            $monthStart
        );
        $this->isPastMonth = $monthStart->lt(Carbon::now()->startOfMonth());

        if ($this->isLocked) {
            // Still load data for display but prevent saving
            $this->loadActivities();
                ...
                ->schema([
                    Forms\Components\Grid::make([
                        ...
                        Forms\Components\TextInput::make(
                            ...
                            ->required()
                            ->helperText(
                                "Колко часа се очаква този работник да работи през " .
                                    $this->getMonthName()
                            )
    ...
    public function save(): void
    {
        ...
        try {
            PresenceConfigurationService::assertMonthUnlocked(
                $this->workplace,
            ...
            foreach ($formData["hours"] as $activityId => $hours) {
                ...
                $permanentActivity = WorkPlaceActivity::where(
                    "work_place_id",
                    $monthlyActivity->work_place_id
                )
                    ->whereNull("date")
                ...
                if ($permanentActivity) {
                    \Log::info(
                        "ConfigureMonthlyHours: Updating PERMANENT activity neto_salary",
                        [
                    ...
                    \Log::warning(
                        "ConfigureMonthlyHours: No permanent activity found, updating monthly activity",
                        [
                            "monthly_activity_id" => $activityId,
                            "new_neto_salary" => $newNetoSalary,
                        ]
    ...
    public function getSubheading(): ?string
    {
        return sprintf(
            "Месец %s · Обект %s",
            $this->monthYear,
            $this->workplaceModel?->name ?? "неизвестен"
        );
    }
    ...
    private function loadActivities(): void
    {
        ...
        $this->activities = WorkPlaceActivity::where(
            "work_place_id",
            $this->workplace
        )
            ->whereNull("date")
    ...
    private function initializeFormData(): void
    {
        ...
        foreach ($this->activities as $activity) {
            ...
            $hoursConfig = HoursActivityByMonth::where(
            ...
            if ($hours && $hours > 0) {
                ...
                $this->data["hourly_rate"][$activity->id] = round(
                    $hourlyRate,
                    4
                );
            } else {
