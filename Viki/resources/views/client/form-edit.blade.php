<form method="PATCH" action="{{route('service.client.update',$client->id)}}" enctype="multipart/form-data" id="edit-form">
@if($errors->any())
	...
	<div class="col form-group">
		...
		<select multiple="multiple" name="region[]" class="form-control" >
				@foreach ($regions as $region)
					 <option value="{{ $region->id }}" @if (in_array($region->id, $arrayRegions)) selected @endif  >
					 {{ $region->name }}
					 </option>
				@endforeach
		</select>
	...
	<div class="col form-group">
		...
		<div class="input-field">
			<input
				...
				pattern ='^\d*(\.\d{1,3})?$'
				max ="1000000"
				value="{{ old('budjet',$client->budget) }}" 
				class="form-control">
		</div>
...
<div class="form-group">
	...
	<select  name="status" class="form-control">
		@foreach ($statuses as $status)	
			<option value="{{ $status['id'] }}" @if($client->status ==  $status['id'] ) selected @endif >
			{{ $status['name'] }}
			</option>
		@endforeach
    </select>
