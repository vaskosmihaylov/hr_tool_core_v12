                ...
                @if($selectedWorkPlace)
                    @if($tableData)
                        ...
                        <div class="presence-content__select">
                            <div class="select col-lg-5 col-sm-7 form-group{{ $errors->has('userWorkPlaces') ? ' has-error' : ''}}" id='userWorkPlaces'>
                            ...
                            <div class="select col-lg-5 col-sm-7 form-group{{ $errors->has('availableMonths') ? ' has-error' : ''}}" id='availableMonths'>
                        ...
                        <div class="card" id="datatableWrapper" style="opacity: 0.0">
                            <div class="card-header">
								 <div>
                                      @if (!$tableIsFinished)
                                        <a href="{{ url('service/presence/deleteWorker',['workPlaceId'=>$selectedWorkPlace->id,'date'=>$selectedDate]) }}" class="btn btn-danger btn-md" title="Изтриване работник"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                      @endif
                                      <a href="{{ url('service/presence/config',['workPlaceId'=>$selectedWorkPlace->id,'date'=>$selectedDate]) }}" class="btn btn-success btn-md" title="Конфигурирай"><i class="fa fa-book" aria-hidden="true"></i></a>
                                      @if ($checkIfAlreadyCopied)
                                          <a href="{{ url('service/presence/addWorker',['workPlaceId'=>$selectedWorkPlace->id,'date'=>$selectedDate]) }}" class="btn btn-success btn-md" title="Добави работник"><i class="fa fa-user" aria-hidden="true"></i></a>
                                          @endif
                                      @if(!$tableIsFinished && $tableNotEditable !== 'true')
                                          <a href="#" data-workPlaceId="{{$selectedWorkPlace->id}}" data-selectedDate="{{$selectedDate}}" id="buttonLockMonth" class="btn btn-success btn-md" title="Приключи месеца"><i class="fa fa-check" aria-hidden="true"></i></a>
                                      @endif
                                      @if ($tableIsFinished && auth()->user()->hasRole('admin'))
                                          <a href="#" data-workPlaceId="{{$selectedWorkPlace->id}}" data-selectedDate="{{$selectedDate}}" id="buttonUnlockMonth" class="btn btn-success btn-md" title="Отключи месеца"><i class="fa fa-key" aria-hidden="true"></i></a>
                                      @endif
                                 </div>
                                @if($tableIsFinished)
                                    <strong>Месеца е приключен!</strong>
                                    <br>
                                    <br>
                                @endif
                                Присъствена форма <strong><span id='today'></span></strong> месец. <span>Бюджет на обекта : <strong><span id="totalUsedBudget">{{$tableData[array_key_first($tableData)]['workPlaceTotalUsedBudget']}}</span> / {{$tableData[array_key_first($tableData)]['workPlaceBudget']}} лв</strong></span>
                            </div>
                                        ...
                                        @foreach($tableData as $tableRow)
                                            ...
                                            @foreach($tableRow['workPlaceActivityWorkers'] as $workPlaceActivityWorker)
                                                <tr>
													...
													<td>{{$workPlaceActivityWorker->egn}}</td>
                                                    @php($allHours = 0)
                                                    ...
                                                    @for($i = 1; $i <= $monthDays; $i++)
                                                        ...
                                                        <td data-workerWorkPlaceActivityId="{{$tableRow['workPlaceActivityId']}}"
                                                            ...
                                                            class="noselect {{$vacationType ? 'vacation'.$vacationType : ""}}  {{$unApprovedClass}} rows-activity-id-{{$tableRow['workPlaceActivityId']}} row-worker-activity-{{$workPlaceActivityWorker->id}}-{{$tableRow['workPlaceActivityId']}} workerHoursData">{{$userDateHours}}</td>
                                                    @endfor
...
<link rel="stylesheet" href="https://cdn.datatables.net/keytable/2.6.2/css/keyTable.dataTables.css">
...
<script>
    ...
    $(document).ready(function() {
        ...
        function saveChanges() {
            ...
            $('.workerHoursData').each(function() {
                var targetElement = $(this);
                totalCost +=  Number(targetElement.text()) * targetElement.attr('data-work-place-activity-hour-price');
            });

            if ($('.workerHoursData').first().attr('data-work-place-budget') < totalCost) {
                var r = confirm("Цената превишава бюджета на обекта");
                if (r != true) {
                    return
                }
            }
            ...
            var postData = {
                workPlaceId : $('#userWorkPlacesSelect').val(),
                date : $('#availableMonthsSelect').val(),
                userData : []
            };

            $('.unsaved-changes').each(function() {

                postData.userData.push(
                    {
                        workPlaceActivityId : $(this).attr('data-workerWorkPlaceActivityId'),
                        workerId : $(this).attr('data-workerId'),
                        day : $(this).attr('data-dateday'),
                        hours : $(this).text(),
                    }
                );
            });

            submitPostData("{{url('service/presence/table/save')}}",postData);
        }

        $('#buttonLockMonth').click(function () {

            var postData = {
                workPlaceId : $(this).attr('data-workPlaceId'),
                date : $(this).attr('data-selectedDate')
            };

            submitPostData("{{url('service/presence/finish')}}",postData);
        });

        $('#buttonUnlockMonth').click(function () {
            ...
            submitPostData("{{url('service/presence/unfinish')}}",postData);
        });
        ...
        if($('#presence').length != 0) {
            ...
            $('.work-place-and-date-control').change(function() {

                var url = "{{url('service/presence/show')}}";
            ...
            var customButtons = [
                ...
                },
                {
                    ...
                    action: function () {
                        window.location.href = "{{url('service/presence/export',['workPlaceId'=>$selectedWorkPlace ? $selectedWorkPlace->id : null,'date'=>$selectedDate])}}"
            ...
            weekDays.forEach(function (weekDay) {
                table.rows().every(function (rowIdx, tableLoop, rowLoop) {
                    var cell = table.cell({ row: rowIdx, column: weekDay + 5 }).node();
        ...
        if (tableNotEditable === 'false') {
            $("td.workerHoursData").click(function() {
                $("td").attr("contenteditable", "false");
             ...
             $("body").keydown(function (e) {
                ...
                if(key == 13) {
                    ...
                    } else if ($(node).hasClass('workerHoursData')) {
                        $("td").attr("contenteditable", "false");
                        $(node).attr("contenteditable", "true");
                        $(node).focus();
        ...
        $('td').on('focus', function (e) {
            selectElementContents($(this).get(0));
        });
        ...
        $(document).on('input', '.workerHoursData', function(event) {
             var element = $(this);

            if (element.attr("data-initial-value") == element.text()) {
                element.removeClass("unsaved-changes");
            } else {
                element.addClass("unsaved-changes");
            }

            var elementWorkPlaceActivityId = element.attr('data-workerWorkPlaceActivityId');
            var workerId = element.attr('data-workerId');
            var pricePerHour = element.attr('data-work-place-activity-hour-price');

            var rowHours = 0;
            $('.row-worker-activity-' + workerId + '-' + elementWorkPlaceActivityId).each(function() {
                rowHours += Number($(this).text());
            });

            if (!isNaN(rowHours)) {

                var numb = rowHours * pricePerHour;
                numb = +numb.toFixed(2);

                $('#total-hours-for-' + elementWorkPlaceActivityId +'-' + workerId).html(rowHours);
                $('#price-for-' + elementWorkPlaceActivityId +'-' + workerId).html(numb);

                var totalHours = 0;

                $('.rows-activity-id-' + elementWorkPlaceActivityId).each(function() {
                    totalHours +=Number($(this).text());
                });
                ...
                if (!isNaN(totalHours)) {
                    ...
                    var numb2 = totalHours * pricePerHour;
                    numb2 = +numb2.toFixed(2);

                    var budgetElement = $('#totalBudgetUsedForId-' + elementWorkPlaceActivityId);
                    budgetElement.html(numb2);
                    if (numb2 <= Number($('#totalBudgetForId-'+ elementWorkPlaceActivityId).text())) {
                        $('#budgetGraphForId-'+ elementWorkPlaceActivityId).css("background-color", "PaleGreen");
                    } else {
                        $('#budgetGraphForId-'+ elementWorkPlaceActivityId).css("background-color", "Tomato");
                    }

                    var hoursElement = $('#totalWorkingUsedHoursForId-' + elementWorkPlaceActivityId);
                    hoursElement.html(totalHours);

                    if (totalHours <= Number($('#totalWorkingHoursForId-'+ elementWorkPlaceActivityId).text())) {
                        $('#hoursGraphForId-'+ elementWorkPlaceActivityId).css("background-color", "PaleGreen");
                    } else {
                        $('#hoursGraphForId-'+ elementWorkPlaceActivityId).css("background-color", "Tomato");
                    }

                    $('.activityTotalPrice').each(function () {

                        var element = $(this);
                        var elementId = element.attr('id');
                        var elementValue = element.text();

                        if (!processedIds.includes(elementId)) {
                            processedIds.push(elementId);
                            totalUsedBudget += Number(elementValue);
                        }
                    });

                    totalUsedBudget = +totalUsedBudget.toFixed(2);

                    $('#totalUsedBudget').text(totalUsedBudget);
                }
            }
        });

        $(".workerHoursData").change(function () {
        ...
        function getWorkingHours (table) {

            var tableData = table.rows().data();
            var monthDays = tableData[0].length - 7;
            var rawData = [];

            table.rows().data().each(function (row) {

                if (row[0] == parseInt(row[0], 10)) {
                    var rowObject = {
                        userId : row[0],
                        workingHours : {}
                    };
                    ...
                    for (var i = 1; i <= monthDays; i++) {
                        rowObject.workingHours[i.toString()] = row[i + 4];
                    }
                    rawData.push(
                        rowObject
                    );
            ...
            rawData = {
                date : $('#availableMonthsSelect').val(),
                workPlace :  $('#userWorkPlacesSelect').val(),
                workerHours: rawData
            };
...
<style>
    ...
    .presence-table table tr td.vacation1,
    .presence-table table tr th.vacation1 {
        background-color: #54ca95;
        color: black;
    }

    .presence-table table tr td.vacation2,
    .presence-table table tr th.vacation2{
        background-color: #00acc1;
        color: black;
    }

    .presence-table table tr td.vacation3,
    .presence-table table tr th.vacation3 {
        background-color: #ee627d;
        color: black;
    }

    .presence-table table tr td.vacation1.day-off,
    .presence-table table tr th.vacation1.day-off {
        background-color: #54ca95;
        color: black;
    }

    .presence-table table tr td.vacation2.day-off,
    .presence-table table tr th.vacation2.day-off{
        background-color: #00acc1;
        color: black;
    }

    .presence-table table tr td.vacation3.day-off,
    .presence-table table tr th.vacation3.day-off {
        background-color: #ee627d;
        color: black;
    }
</style>