...
class ClientController extends Controller
{
	...
	public function viewFormClient()
	{
		...
		if (Auth::user()->hasRole('manager')) {
			//$regions = Region::where('id','=',VikiUser::getCurrentUserRegionId(Auth::user()->id))->get();
    ...
    public function edit($id)
    {
		...
		foreach($client->regions as $region){
				array_push($arrayRegions,$region->id);
		}
		
		$statuses = Client::clientStatuses();
		if (Auth::user()->hasRole('manager')) {
			//$regions = Region::where('id','=',VikiUser::getCurrentUserRegionId(Auth::user()->id))->get();
			$regions = Region::whereIn('id', VikiUser::getCurrentUserRegionId(Auth::user()->id));
		}else{
			$regions = Region::where('status','=', Region::REGION_ACTIVE)->get();
		}
    ...
    public function update(ClientRequest $request, $id)
    {
		try{
		    ...
		    activity()->performedOn($client)
				->causedBy(Auth::user())
				->withProperties(['customProperty' => 'customValue'])
				->log('редактин клиент: '.$client->name);
